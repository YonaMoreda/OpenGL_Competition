 ...
 */
void Model::unitize() {
    QVector3D minPoint(10000, 10000, 10000);
    QVector3D maxPoint(0, 0, 0);

    for (int i = 0; i != vertices.size(); ++i) {
        minPoint.setX(std::min(minPoint.x(), vertices[i].x()));
        minPoint.setY(std::min(minPoint.y(), vertices[i].y()));
        minPoint.setZ(std::min(minPoint.z(), vertices[i].z()));

        maxPoint.setX(std::max(maxPoint.x(), vertices[i].x()));
        maxPoint.setY(std::max(maxPoint.y(), vertices[i].y()));
        maxPoint.setZ(std::max(maxPoint.z(), vertices[i].z()));
    }

    QVector3D box;
    box.setX(fabs(maxPoint.x() - minPoint.x()));
    box.setY(fabs(maxPoint.y() - minPoint.y()));
    box.setZ(fabs(maxPoint.z() - minPoint.z()));

    float longestEdge = std::max(std::max(box.x(), box.y()), box.z());

    float scaleFactor = (float) 5 / longestEdge;

    QVector3D centerPoint;
    centerPoint.setX( (maxPoint.x() + minPoint.x()) / 2);
    centerPoint.setY( (maxPoint.y() + minPoint.y()) / 2);
    centerPoint.setZ( (maxPoint.z() + minPoint.z()) / 2);
